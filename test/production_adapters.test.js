...
describe('protected live dashboard', () => {
  test('bundles the byte-verified reviewed design and preserves the fail-safe console', async () => {
    ...
    assert.doesNotMatch(html, /class="panel decision-panel"/u);
    assert.match(html, /Layer 5–6 · Structure and governance/u);
  ...
  test('ships syntactically valid live bindings for every protected shadow surface', () => {
    ...
    assert.doesNotMatch(source, /function marketReadiness\(/u);
    ...
    assert.doesNotMatch(source, /#decisions \.decision-panel/u);
